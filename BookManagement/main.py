...
def log_handle():
    ...
    if request.method == 'POST':
        ...
        if username=="" or password=="":
            return flask.render_template("log_in_fail.html")
        # 对密码进行md5处理
    ...
    # 通过mysql进行存储
    db = pymysql.connect(host="localhost", user="root", password="201922", db="www")
...
def log_success():
    db = pymysql.connect(host="localhost", user="root", password="201922", db="book_manager")
    cursor = db.cursor()
...
def books_add():
    ...
    # 创建Connection连接
    conn = pymysql.connect(host='localhost', port=3306, db='book_manager', user='root', password='201922',
                           charset='utf8')
    # 获得Cursor对象
    cs1 = conn.cursor()
